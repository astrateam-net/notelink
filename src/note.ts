...
export default class Note {
  ...
  async share() {
    ...
    try {
      ...
      bodyStyle.classes.push('notelink-plugin') // Add a targetable class for published notes
      ...
      this.css = this.cssRules
        .filter((rule: CSSMediaRule) => {
          /*
          /*
          Remove styles that prevent a print preview from showing on the web.
    ...
    for (const el of this.contentDom.getElementsByClassName('callout')) {
      ...
      if (svgEl) {
        svgEl.outerHTML = `<svg width="16" height="16" data-notelink-lucide="${icon}"></svg>`
      }
    }
